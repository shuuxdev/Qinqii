...
import { QinqiiPostImage, QinqiiPostVideo } from './CommonComponent';
...
export const ImageGrid = ({ attachments }) => {
    ...
    const renderAttachments = () => {
        ...
        return attachments.slice(0, 4).map((attachment, index) => {
            if(index === 3 && cnt > 4)
            {
                return <LastGridItem cnt={cnt - 4} attachment={attachment} index={index} openMediaViewerModal={openMediaViewerModal}/>
            }
            return <GridItem attachment={attachment} index={index} openMediaViewerModal={openMediaViewerModal}/>
        });
    };
